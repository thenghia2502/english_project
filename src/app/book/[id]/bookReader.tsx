...
export function BookReader({ book }: BookReaderProps) {
  ...
  useEffect(() => {
    const loadNote = async () => {
      try {
        const response = await fetch(`http://localhost:4000/api/notes/${book.id}`)
        if (response.ok) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 border-r bg-muted/30">
          <ScrollArea className="h-full">
            <div className="p-4">
              <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Units</h2>
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="border-b p-4 h-full flex flex-col">
              <div className="mb-3 flex-1">
                <NotesEditor
                  value={notes[selectedUnit.id] || ""}
                  onChange={(content) => setNotes({ ...notes, [selectedUnit.id]: content })}
